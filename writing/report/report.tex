...
\todo{Further motivation: Extend to larger groups.}
...
\todo{Explain that invalid frames are dropped.}
...
\todo{Note that we did not investigare further because the focus is the social model.}
...
\todo{Note that fish swim near the wall most of the time. Maybe add density plot?}
...
\todo{Make clear that bins are for all variables, and not seperate for variables.}
\todo{Recompute bin mean/stds!}
...
\todo{Add smaller plot showing grid without bins on the corners.}
\begin{figure}[htb]
...
\todo{Maybe make clear that this algorithm is own work?}
...
\todo{Replace heading change with trajectory vector or similar.}
\begin{marginfigure}
  \begin{tikzpicture}[scale=0.5]
...
\todo{Jacob: Is this due to the vanishing gradient with relu units?}
...
The number of components for the \textsc{mdn}-networks was set to 5.\todo{Jacob marked this, why?}
...
\section{Evaluation}
We start by comparing the angle prediction from the wall model against the baseline of always predicting the mean angle.\todo{See note in pdf!}
...
\todo{Null model, NN models on top, linear bottom.}
\begin{table}[htb]
\centering
...
\item The explicit modeling of probability distributions allows us to sample from the trajectory distribution conditioned on the social surroundings and is able to predict bi-modal distributions.
  \todo{Jacob: This is also biologically relevenat, since we observe often non-gaussian or bi-modal prob.\ distr.
  Force based models dont capture that.}
