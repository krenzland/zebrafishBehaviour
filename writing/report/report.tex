...
This paper is concerned with the development of models of the behavior of juvenile zebrafish.
...
Furthermore, we build our models such that they easily extend to larger groups.
...
We convert the video to a constant frame rate by iterating over the frames and inserting frames between them if the time between frames differs by more than \SI{0.005}{\s} from a constant frame rate.
These inserted frames are marked invalid as well.
Invalid frames are later discarded.
...
We also use it to compute the derivative of the velocity.
...
We remove a kick if it contains a timestep which was marked as invalid.
This segmentation procedure results in ca.\ 148000 kicks.
We use the first 80\% of each experiment for training, the remaining 20\% are used as testing set.
...
\end{equation*}
where $r_w$ corresponds to the fish and $\theta_w$ is its relative angle towards the wall~\footfullcite{calovi}.
...
\sidenote[][-1cm]{We found that the functional form of \citeauthor{calovi} does not work as well in our case.
...
For the sake of brevity, we do not investigate the wall model further as the focus of this work is modeling the social interactions.
...
This corresponds to a wall distance of ca.\ \SI{4.8}{\cm}.
The number of kicks in the training set is reduced by this to ca.\ 19400.
Note that we discard a majority of our data because juvenile zebrafish mostly swim near the boundaries of the arena.

Our social models use a receptive field (\textsc{rf}) as their input~\autocite{discreteModes}.
We construct this \textsc{rf} by rotating the coordinate system such that the fish we are considering is parallel to the x-axis and looks to the right.
...
\begin{marginfigure}
 ...
 Note that the focal fish (red) looks to the right and is at the origin.}
\label{fig:rf}
\end{marginfigure}
...
For interpretability, it is useful to be able to clearly distinguish situations where the other fish is behind or in front of the focal fish and whether it is on its left or right side.
Our proposed algorithm to compute the bin edges is shown in algorithm~\ref{alg:binning}.
It first divides the x-axis (with symmetry around the origin) into bins that each have a roughly equal number of fish and then subdivides each of these bins in the same manner.
...
\EndFunction 
\Require{Positions of focal fish $(\bm{x}, \bm{y})$ and number of bins per axis $N$}
...
The bins are chosen such that they have equal size.
...
\begin{figure}[tb]
  ...
  \includegraphics{rf_occupancy_test_zoomed_paper}
   \centering
   \caption{Number of data points in each bin obtained from data-driven binning scheme, evaluated on testing set.
     ...
     The left image shows all bins, the right image shows the $6x6$ neighborhood of the origin.
...
This leads to $3 \times 64 = 192$ features for the discretization with 64 bins.
...
\begin{marginfigure}
 ...
 We model the trajectory vector of the red fish given the current position and angle of the other fish.}
\label{fig:no-memory}
\end{marginfigure}

The linear model without memory for the prediction of a component \(i\) of our 2-dimensional target vector \(\bm{y}\) can be written as
\begin{align*}
  \hat{\bm{y^i}}  &= \bm{X_0} \bm{\beta^i_0} + \text{intercept} + \varepsilon \\
  \varepsilon &\sim \mathcal{N}\left(0, \sigma^2 \right),
\end{align*}
where \(\bm{X_0}\) is the design matrix at kick-off time and \(\bm{\beta^i_0}\) is a vector of learned weights.
The error $\varepsilon$ follows the normal distribution \(\mathcal{N}(0, \sigma^2)\) with mean zero and a residual variance of \(\sigma^2\).
...
Furthermore all linear models discussed in this work are trained using a $L_2$ regularized mean squared error (\textsc{mse}) loss.
This type of model is known in statistics as ridge regression.
...
\begin{marginfigure}[4cm]
 ...
 \caption{Situation considered for the models with memory:
   We model the trajectory vector of the red fish given the trajectory of the other fish.
...
\begin{equation*}
  \hat{\bm{y}} = \sum_t c_t \bm{X_t} \bm{\beta} + \text{ intercept} + \varepsilon.
\end{equation*}
...
The complete model is then an arbitrary combination of the encoders and decoders (figure~\ref{fig:nn-models}).

\begin{marginfigure}
  \centering
    \includegraphics[width=1\textwidth]{rf_encoder_decoder}
    \caption{General encoder-decoder architecture.
      Symbols $\approx$ and $\mapsto$ correspond to encoding and decoding. Blue is input, gray hidden and black output.}
    \label{fig:nn-models}
\end{marginfigure}

We describe two simple encoders (figure~\ref{fig:encoders}):
\begin{itemize}
...
\item
  As a temporal model we consider a standard recurrent neural network (\textsc{rnn}) which has an linear transformation of the input and a recurrent hidden-to-hidden connection.
  ...
  These two layers are added and transformed with the \textit{tanh} non-linearity
  \begin{equation*}
  ...
  We learn the initial hidden state \(\bm{h_0}\) instead of using a zero-initialized state as it improved the speed of convergence.
...
Note that both models (disregarding the learned initial \textsc{rnn}-state) have the same number of parameters.
 ...
 \begin{figure}[h]
   \centering
  \includegraphics[width=0.4\textwidth]{mlp_encoder}\qquad%
  \includegraphics[width=0.4\textwidth]{rnn_encoder} 
  \caption{%
    \textbf{Left}: Architecture of \textsc{mlp} encoder. Symbol $d(\ast)$ corresponds to linear layer, followed by tanh and dropout. \newline
    \textbf{Right}:
    Architecture of \textsc{rnn} encoder. Symbol $\ast$ is linear layer, $d(\,)$ is recurrent dropout, $+$ is summation and $f$ is \textit{tanh}.\newline
...
Note that we can write the Cholesky decomposition\autocite{numericalRecipes}%
...
\item The diagonals of all \(\bm{L}_i\) need to be larger than zero.
  ...
  Additionally it ensures that the variance is strictly positive and thus avoids degenerate distributions.}
...
We predict the Gaussian mixture parameters each by computing a linear transformation of the hidden state of the encoder followed by the appropriate non-linearity (figure~\ref{fig:mdn}).
...
\begin{marginfigure}
  \centering
  \includegraphics[width=0.75\textwidth]{mdn_decoder}
  \caption{Architecture of \textsc{mdn} decoder. Symbol $\ast$ is linear layer. Gray is hidden state and black are the outputs.}
  \label{fig:mdn}
\end{marginfigure}
...
\sidenote[][0.3cm]{We compute the determinant directly from the Cholesky matrix \(\bm{L_i}\).
...
For the diagonal case, both determinant and inverse are trivial to compute and numerically stable.\vspace{0.3cm}}.
...
We set the number of components for the \textsc{mdn}-networks to 5.
We use stochastic gradient descent with an initial learning rate of 0.1, a weight decay of 0.001 and a Nesterov momentum of 0.9 to optimize our networks.
...
An alternative approach would be considering the representation of the angles as unit vectors and computing their \textsc{mse}.
Computing the error on angles follows the optimization problem used to optimize the models more closely and is thus more appropriate.
...
\end{equation*}
Due to symmetry, the mean angle of ca. 0.007 is nearly zero.
...
The reason for this is that the regularized \textsc{mse} loss function used for ridge regression can be derived by computing the \textsc{nll} under the assumption that the likelihood is Gaussian and that we have a Gaussian prior on the parameters of the model\autocite{statLearning}.
...
Note that it is not straightforward to use more complicated methods such as log-likelihood ratios or the Akaike information criterion because we cannot estimate the number of degrees of freedoms for penalized recurrent neural network models.

The best \textsc{nll}-error was achieved by the \textsc{rnn-mdn} and \textsc{mlp-mdn} models on the training and testing set respectively (table~\ref{tab:results-social}).
Interestingly, the \textsc{rnn-mdn} methods had the best testing \textsc{mse}, even though we include models that were explicitly trained with the \textsc{mse}-loss.
All linear models had larger errors for all considered metrics but did not achieve a significantly worse \textsc{mse}.
...
\item Because the social models do not use properties of other animals directly but rather model the mean features per bin they extend trivially to larger animal groups, without changing the statistical models or the spatial discretization.
  ...
  We believe that the models developed here would perform comparatively much better for larger group sizes than force-based models, as this would result in less sparse input vectors.
\item The explicit modeling of probability distributions allows us to sample from the trajectory distribution conditioned on the social surroundings and is able to predict bi-modal distributions.
  This is biologically relevant, since we often observe non-Gaussian distributions in experiments.
  Force based models do not capture these distributions.
